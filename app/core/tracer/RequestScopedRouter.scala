...
/**
  ...
  */
class RequestScopedRouter[T <: Router](injector: Injector,
                                       prefix: String = "/",
                                       modules: Seq[_ >: AbstractModule] =
                                         Seq.empty)(
    implicit routerClassTag: ClassTag[T])
    extends Router
    with RequestScopedRouterProvider {

  def this(injector: Injector,
           routerClass: Class[T],
           prefix: String,
  ...
  def routerForRequest(request: RequestHeader): Router = {
    ...
    val requestScopedModules = modules.map { clazz =>
      injectorWithRequest.getInstance(clazz.asInstanceOf[Class[AbstractModule]])
    }
  ...
  def withPrefix(prefix: String): RequestScopedRouter[T] =
    new RequestScopedRouter(injector, prefix, modules)
