...
object AddressModel {
  ...
  implicit val addressFormat: Format[AddressModel] =
    Jsonx.formatCaseClass[AddressModel]

}

case class Produit(code: String,
                   label: String,
                   flag: Option[Boolean] = Some(false))
...
object InstructionNonLivraison {
  ...
  implicit val enum: Enum[InstructionNonLivraison] =
    Enum.derived[InstructionNonLivraison]
  implicit val format: Format[InstructionNonLivraison] =
    JsonImplicits.enumFormat(enum)
...
object CaracteristiquesModel {
  import ai.x.play.json.Jsonx
  implicit val format: Format[CaracteristiquesModel] =
    Jsonx.formatCaseClass[CaracteristiquesModel]
}

case class MontantDesOptions(tarifDesOptions: Seq[TarifOption]) {
  def findMontantOption(codeOption: String): Option[Double] =
    tarifDesOptions.find(_.option.codeOption == codeOption).flatMap(_.montantHT)
}
object MontantDesOptions {
  implicit val format: Format[MontantDesOptions] =
    Json.format[MontantDesOptions]
}
...
object EtiquetteModelAdapter {
  ...
  import ai.x.play.json.Jsonx
  implicit val format: Format[EtiquetteModelAdapter] =
    Jsonx.formatCaseClass[EtiquetteModelAdapter]
}
